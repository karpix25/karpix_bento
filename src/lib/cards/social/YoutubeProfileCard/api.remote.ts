...
export const fetchYoutubeProfile = query(
	'unchecked',
	async (channelUrl: string): Promise<YoutubeProfileData | undefined> => {
			...
			const profile: YoutubeProfileData = {
				id:
					data.responseContext?.serviceTrackingParams?.[0]?.params?.find(
						(p: any) => p.key === 'browse_id'
					)?.value || '',
				title: header.title?.simpleText || header.title?.runs?.[0]?.text || '',
				avatar:
					header?.content?.pageHeaderProcessor?.profilePicRenderer?.image?.thumbnails?.[0]?.url ||
					header.avatar?.thumbnails?.[0]?.url ||
					'',
				subscriberCountText:
					header.subscriberCountText?.simpleText ||
					header.metadata?.contentMetadataRenderer?.metadata?.[1]?.metadata?.contentMetadataRenderer
						?.metadata?.[0]?.simpleText ||
					'0 subscribers',
				handle:
					header.metadata?.contentMetadataRenderer?.metadata?.[0]?.metadata?.contentMetadataRenderer
						?.metadata?.[0]?.simpleText || '',
				url: channelUrl
			};
